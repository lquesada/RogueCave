...
public class InventoryController extends Controller {
	...
	private static InventoryController singleton;
	public static Controller get() {
		if (singleton==null)
			singleton = new InventoryController();
		return singleton;
	}
	...
	public void keyPress(int eventKey) {
		...
		if (eventKey==RogueCaveGUI.config.getKey(ControlID.INVENTORY)) { RogueCaveGUI.newController = IngameController.get(); }
	...
	public void mouseRelease(int x, int y) {
		...
		if (!dragging) {
			...
			RogueCaveGUI.game.getPlayer().getInventory().touch(selSlot);
			if (overItem != null && originSlot==selSlot && !RogueCaveGUI.game.getPlayer().getInventory().isComboSelfSlot(originSlot)) {
				RogueCaveGUI.game.getPlayer().getInventory().replaceItem(overItem);
			}
			if (overItem != null && originSlot==selSlot && RogueCaveGUI.game.getPlayer().getInventory().isEquipmentSlot(originSlot)) {
				RogueCaveGUI.game.getPlayer().getInventory().move(originSlot,Inventory.NONE);
			}
	...
	private void drawPlayerStats() {
		int stw = RogueCaveGUI.screenWidth+RogueCaveGUI.config.playerStatsBoxX-RogueCaveGUI.menuRightMargin;
		int sth = RogueCaveGUI.config.playerStatsBoxY-47+RogueCaveGUI.menuTopMargin;
		...
		Stats sNew = null;
		if (overItem != null) {
			if (RogueCaveGUI.game.getPlayer().getInventory().isEquipmentSlot(selectedSlot(RogueCaveGUI.mouseX,RogueCaveGUI.height-RogueCaveGUI.mouseY))) {
				sNew = RogueCaveGUI.game.getPlayer().getStatsWithRemove(selectedSlot(RogueCaveGUI.mouseX,RogueCaveGUI.height-RogueCaveGUI.mouseY));
			}
			else
				sNew = RogueCaveGUI.game.getPlayer().getStatsWithReplace(overItem,RogueCaveGUI.game.getPlayer().getInventory().getItem(RogueCaveGUI.game.getPlayer().getInventory().accompanyingSlot(selectedSlot(RogueCaveGUI.mouseX,RogueCaveGUI.height-RogueCaveGUI.mouseY))));
		}
	...
	private void drawEquipmentBox() {
		int stw = RogueCaveGUI.screenWidth+RogueCaveGUI.config.equipmentBoxX-RogueCaveGUI.menuRightMargin;
		int sth = RogueCaveGUI.config.equipmentBoxY-47+RogueCaveGUI.menuTopMargin;
		glViewport(stw,RogueCaveGUI.height-sth-158,121,158);
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glOrtho(0, 121,158, 0, 1.5, -1.5);
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		drawSpriteGraphic(Resources.getGraphic(GraphicID.GUIEquipmentBackground), 0,RogueCaveGUI.height, 0);

		drawInventorySlot(4,4,Inventory.AMULET,GraphicID.GUIEmptyAmulet);
		drawInventorySlot(42,4,Inventory.HELMET,GraphicID.GUIEmptyHelmet);
		drawInventorySlot(80,42,Inventory.ARMS,GraphicID.GUIEmptyArms);
		drawInventorySlot(42,42,Inventory.ARMOR,GraphicID.GUIEmptyArmor);
		drawInventorySlot(42,80,Inventory.LEGS,GraphicID.GUIEmptyLegs);
		drawInventorySlot(42,118,Inventory.BOOTS,GraphicID.GUIEmptyBoots);
		drawInventorySlot(4,80,Inventory.RING1,GraphicID.GUIEmptyRing);
		drawInventorySlot(80,80,Inventory.RING2,GraphicID.GUIEmptyRing);
	}

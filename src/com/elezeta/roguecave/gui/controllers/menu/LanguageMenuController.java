...
public class LanguageMenuController extends MenuController {

	private static LanguageMenuController singleton;
	public static Controller get() {
		if (singleton==null)
			singleton = new LanguageMenuController();
		return singleton;
	}

	private LanguageMenuController() { }
	...
	public void activate(Controller oldController) {
		...
		text1.setX(0);
		text1.setY(-165-15);
		text1.setString(RogueCaveGUI.language.getString(StringID.CHOOSELANGUAGE));
		...
		for (i = 0;i < values.length;i++) {
			...
			b.setNumber(i);
			b.setEnabled(true);
			b.setString(LanguageFactory.generateLanguage(values[i]).getString(StringID.LANGUAGENAME));
			final LanguageID lan = values[i];
			b.setActuator(new MenuActuator(){
				@Override
				public void run() {
					RogueCaveGUI.config.language = lan;
	...
	@Override
	public String getName() { return "LanguageMenu"; }
